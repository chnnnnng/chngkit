...
class Crawler:
    ...
    def getIcs(self,kb):
        kbs = json.loads(kb.text).get('kbList')
        resl = []
        for course in kbs:
            zcd = course.get('zcd')
            zcd = zcd.replace('周', '')
            ...
            if x == -1:
                ...
                beginWeek = int(zcd[0:x])
                endWeek = int(zcd[x+1:len(zcd)])
            jcs = course.get('jcs')
            y = jcs.find('-')
            if y == -1:
                startTime = int(jcs)
                ...
                startTime = int(jcs[0:y])
                endTime = int(jcs[y+1:len(jcs)])
            resl.append({
                'name':course.get('kcmc'), #课程名称
                'startWeek':int(beginWeek), #上课周数
                'endWeek':int(endWeek),
                'location':course.get('cdmc'), #上课地点
                'isDouble':isDouble, #是否是隔周
                'teacher':course.get('xm'),
                'week':int(course.get('xqj')),
